...
v_c <- n1 %>% ungroup() %>% filter(n_uttered>n_size*n_char) %>% pull(char_unique) %>% unique()
...
```{r}
df_res %>% group_by(chr,book_id,char_samples,sample_id) %>% filter(char_samples>10) %>% summarise(acc = mean(pred)) %>% group_by(chr,book_id,char_samples) %>%  summarise(acc=mean(acc)) %>% mutate(year = as.integer(str_extract(book_id,"[0-9]{4}"))) %>% ggplot(aes(year, acc)) + geom_point(alpha=0.5) + geom_boxplot(aes(group=book_id)) + geom_smooth(method="lm")
```
